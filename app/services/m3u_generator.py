...
def generate_m3u_playlist(
    ...
    Args:
        ...
        wanted_stream_ids: List of stream IDs to include (optional)
        unwanted_stream_ids: List of stream IDs to exclude (optional)
    ...
    wanted_stream_ids_set = {str(stream_id).strip() for stream_id in (wanted_stream_ids or []) if str(stream_id).strip()}
    unwanted_stream_ids_set = {str(stream_id).strip() for stream_id in (unwanted_stream_ids or []) if str(stream_id).strip()}
    ...
    if include_vod:
        ...
        if series_streams:
            ...
            for stream in series_streams:
                ...
                should_fetch = True
                stream_identifier = str(stream.get("stream_id") or stream.get("series_id") or "").strip()
                has_wanted_filters = bool(wanted_patterns or wanted_stream_ids_set)
                if has_wanted_filters:
                    if wanted_stream_ids_set:
                        should_fetch = stream_identifier in wanted_stream_ids_set
                    else:
                        should_fetch = any(
                            group_matches(category_name, w) or group_matches(group_title, w)
                            for w in (wanted_groups or [])
                        )
                if should_fetch and (unwanted_patterns or unwanted_stream_ids_set):
                    should_fetch = not (
                        any(
                            group_matches(category_name, u) or group_matches(group_title, u)
                            for u in (unwanted_groups or [])
                        )
                        or stream_identifier in unwanted_stream_ids_set
                    )
    ...
    for stream in streams:
        ...
        if has_wanted_filters:
            if wanted_stream_ids_set:
                include_stream = stream_identifier in wanted_stream_ids_set
            else:
                include_stream = any(
                    group_matches(category_name, wanted_group)
                    or group_matches(group_title, wanted_group)
                    for wanted_group in (wanted_groups or [])
                )
        else:
            include_stream = True

        if include_stream and (unwanted_patterns or unwanted_stream_ids_set):
            include_stream = not (
                any(
                    group_matches(category_name, unwanted_group)
                    or group_matches(group_title, unwanted_group)
                    for unwanted_group in (unwanted_groups or [])
                )
                or stream_identifier in unwanted_stream_ids_set
            )
