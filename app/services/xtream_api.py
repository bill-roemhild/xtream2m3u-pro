...
def normalize_user_info(user_info):
    """Normalize user_info shape for UI/API stability."""
    user_info = user_info or {}
    return {
        "username": _to_str(user_info.get("username")),
        "status": _to_str(user_info.get("status")),
        "exp_date": _to_int(user_info.get("exp_date"), 0),
        "created_at": _to_int(user_info.get("created_at"), 0),
        "is_trial": _to_bool(user_info.get("is_trial")),
        "active_cons": _to_int(user_info.get("active_cons"), 0),
        "max_connections": _to_int(user_info.get("max_connections"), 0),
        "allowed_output_formats": user_info.get("allowed_output_formats") or [],
    }


def normalize_server_info(server_info):
    """Normalize server_info shape for UI/API stability."""
    server_info = server_info or {}
    return {
        "url": _to_str(server_info.get("url")),
        "port": _to_int(server_info.get("port"), 0),
        "https_port": _to_int(server_info.get("https_port"), 0),
        "server_protocol": _to_str(server_info.get("server_protocol"), "http"),
        "timezone": _to_str(server_info.get("timezone")),
        "timestamp_now": _to_int(server_info.get("timestamp_now"), 0),
        "time_now": _to_str(server_info.get("time_now")),
    }


def normalize_channel_item(item):
    """Normalize a stream/movie/show item for consistent API consumers."""
    item = item or {}
    return {
        "stream_id": _to_int(item.get("stream_id") or item.get("series_id") or item.get("num"), 0),
        "name": _to_str(item.get("name")),
        "category_id": _to_str(item.get("category_id")),
        "epg_channel_id": _to_str(item.get("epg_channel_id")),
        "stream_icon": _to_str(item.get("stream_icon")),
        "content_type": _to_str(item.get("content_type")),
        "raw": item,
    }
...
def fetch_xtream_action(url, username, password, action, timeout=30, **params):
    """Fetch a generic Xtream player_api action and normalize error handling."""
    query = {
        "username": username,
        "password": password,
        "action": action,
    ...
    api_url = f"{url}/player_api.php?{urllib.parse.urlencode(query)}"
    return fetch_api_data(api_url, timeout=timeout)


def fetch_short_epg(url, username, password, stream_id, limit=6):
    """Fetch short EPG for a stream."""
    data = fetch_xtream_action(
        url,
        username,
        password,
        "get_short_epg",
        stream_id=stream_id,
        limit=_to_int(limit, 6),
        timeout=20,
    )
    return data


def fetch_full_epg(url, username, password, stream_id):
    """Fetch full EPG for a stream."""
    data = fetch_xtream_action(
        url,
        username,
        password,
...
def fetch_vod_info(url, username, password, vod_id):
    """Fetch VOD/movie details."""
    return fetch_xtream_action(url, username, password, "get_vod_info", vod_id=vod_id, timeout=30)


def fetch_series_info(url, username, password, series_id):
    """Fetch series/show details."""
    return fetch_xtream_action(url, username, password, "get_series_info", series_id=series_id, timeout=30)


def fetch_catalog(url, username, password, content_type="live", category_id=None, page=1, limit=200):
    """Fetch paginated catalog items from live/vod/series streams."""
    action_map = {
    ...
    mapped_type = (content_type or "live").lower()
    action = action_map.get(mapped_type, "get_live_streams")
    data = fetch_xtream_action(url, username, password, action, category_id=category_id, timeout=90)
    if isinstance(data, tuple):
        return data
    if not isinstance(data, list):
        return []

    start = max(_to_int(page, 1) - 1, 0) * max(_to_int(limit, 200), 1)
    end = start + max(_to_int(limit, 200), 1)
    paginated = data[start:end]
    normalized = []
    content_label = "vod" if action == "get_vod_streams" else ("series" if action == "get_series" else "live")
    for item in paginated:
        item["content_type"] = content_label
        normalized.append(normalize_channel_item(item))

    return {
        "content_type": content_label,
        "page": max(_to_int(page, 1), 1),
        "limit": max(_to_int(limit, 200), 1),
        "total": len(data),
        "items": normalized,
    }
...
def fetch_categories_and_channels(url, username, password, include_vod=False, include_all_streams=False):
    ...
    try:
        ...
        if include_vod:
            ...
            vod_for_m3u = request.endpoint == 'api.generate_m3u' or include_all_streams
            if vod_for_m3u:
